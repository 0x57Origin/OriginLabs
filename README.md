...
- [Sysmon Deployment](Labs/Sysmon_Deployment.md) - I installed Sysmon with a config, confirm events are landing, then read a process-create event field by field. Ran a  encoded PowerShell command, caught it in the log by filtering for `-enc`, and decoded it to prove intent. It is a beginner version but it does cover a lot of the Sysmon commands. P.S it logs more than you would think. 
